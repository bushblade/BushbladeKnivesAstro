...
- **Gallery grid thumbs fade in on load**: they start with `opacity-0 text-transparent` (so the generic fallback alt text doesn't flash) and are revealed by a hydration-safe effect in `Gallery.tsx` — already-cached images (`img.complete`) are revealed instantly, the rest fade in via a `{ once: true }` `load` listener. The image's `transition-property` list must include `transform`, `scale`, **and** `opacity`: Tailwind v4's `scale-*` utilities use the native `scale` property, so a list that omits `scale` makes the hover zoom snap (while the opacity fade still animates). A photo's `alt` is optional in `buildGalleryPhotos` sources (`{ name, alt?, src }`) — it falls back to the filename. Dev serves images from `/_image` on-demand via sharp (first encode is slow); production pre-generates avif, so slow first loads are a dev-only artifact.
