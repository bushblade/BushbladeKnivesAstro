...
- **Headings**: use the site's heading scale — section `h2` = `text-3xl`, sub-heading `h3` = `text-2xl`, captions `h4` = `text-center italic`. Headings are default weight (400): no `font-medium`, no arbitrary rem sizes (e.g. `text-[1.73rem]`). The page title is `PageTitle` (Bilbo), not a bare `h1`.
