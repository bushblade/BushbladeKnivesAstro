...
- **Gallery lightbox must never upscale images**: `react-touch-drag-slider` sizes every slide to the full viewport and its CSS permits `img { max-width: 100% }`, so low-res sources (e.g. the ~400–480px portrait files in `src/images/me/`) get stretched and look pixelated. The lightbox `<img>` in `src/components/Lightbox.tsx` keeps a dual cap — `maxWidth: min(${photo.width}px, 100%)` and `maxHeight: min(${photo.height}px, 100%)` (native source dimensions from `buildGalleryPhotos`) — so it's bounded by BOTH its native resolution (never upscaled) AND the slide container (`100%`). A naive `maxWidth: photo.width` / `maxHeight: '1000px'` cap overrides the library's container-relative `max-width: 100%`, letting the img size itself to the viewport via `sizes="100vw"`/srcset — that caused portrait/landscape cropping (img taller than the viewport in landscape) and adjacent slides peeking at the image's sides during window resize. `.rtds-single-slide-styles { overflow: hidden }` in `src/styles/global.css` is a defensive belt-and-suspenders so no slide's content can bleed into its neighbours.
